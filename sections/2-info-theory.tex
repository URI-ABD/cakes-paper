...
The following analysis assumes that the \textbf{distance function} is a metric;
...
See Section~\ref{sec:discussion} for a discussion on what happens with distance functions for which these properties do not hold.
...
The \textbf{Metric Entropy} (Kolmogorov Entropy) of the dataset, denoted by $N_{\epsilon}^{ent}(X)$, is the maximum $k$ such that $\{x_1, \dots, x_k\} \subseteq X$ and $f(x_i, x_j) \geq \epsilon \ \forall \ i \neq j$, i.e. the maximum number of $\epsilon$-separated instances in the dataset.

The \textbf{Internal Covering Number} of the dataset, denoted by $N_{\epsilon}^{int}(X)$, is the minimum $k$ such that $\{x_1, \dots, x_k\} \subset X$ and $\bigcup_{i = 1}^{k} B(c_i, \epsilon) = X$, i.e. the minimum number of $\epsilon$-radius clusters that collectively cover $X$.

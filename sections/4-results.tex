...
In these plots, we label the recall for algorithms which do not exhibit perfect recall;
if no such labels are given for an algorithm, then that algorithm exhibits perfect recall.


Tables \ref{table:results:ann-fashion}, \ref{table:results:ann-glove-25}, \ref{table:results:ann-sift}, and \ref{table:results:ann-random} show the throughput and recall of CAKES's algorithms at each cardinality multiplier for the fashion-mnist, glove-25, sift, and random datasets respectively.
...
In Figure~\ref{fig:results:fashion-mnist-scaling}, which shows results for fashion-mnist, we observe that as the multiplier increases, the CAKES algorithms (GreedySieve in blue, RepeatedRnn in green, and Sieve in red) exhibit higher throughput than na\"{i}ve linear search, plotted in orange.
We also observe that CAKES outperforms FAISS-IVF (plotted in pink) for all multipliers greater than one.
Though state-of-the-art algorithms HNSW (plotted in gray), and ANNOY (plotted in purple) have higher throughput than CAKES for all cardinalities, we note that CAKES always exhibits perfect recall while HNSW and ANNOY exhibit much lower recall, as shown in Table~\ref{table:results:ann-fashion}.
...
Meanwhile, while CAKES exhibits perfect recall.
...
With glove-25 (Figure~\ref{fig:results:glove-25-scaling}), we observe that each of CAKES' algorithms outperforms linear search at high multipliers.
Similar to the results for fashion-mnist, we see that CAKES outperforms FAISS-IVF for all multipliers greater than four.
We see once again that HNSW and ANNOY have higher throughput than CAKES for all multipliers, CAKES exhibits better recall at all multipliers, as shown in Table~\ref{table:results:ann-glove-25}.
...
In Figure~\ref{fig:results:sift-scaling}, which displays the scaling behavior of algorithms on the sift dataset, we observe that while linear search performs best for low multipliers, our algorithms begin to outperform linear search at a multiplier of eight.
Again, we observe that CAKES outperforms FAISS-IVF for all multipliers greater than four. 
For this dataset, Sieve, plotted in red, is the highest throughput CAKES algorithm for all multipliers greater than right.
...
With the random dataset, as seen in Figure~\ref{fig:results:random-scaling}, we observe that the three CAKES algorithms exhibit the worst throughput out of all algorithms, at all multipliers. 
As with the real datasets, HNSW and ANNOY exhibit the highest throughput of any of the algorithms and CAKES exhibits perfect recall at all multipliers. 

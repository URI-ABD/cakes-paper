...
{\color{red} TODO: revisit this sentence with cosine dist plot in mind: In these plots, we label the recall for algorithms which do not exhibit perfect recall; if no such labels are given for an algorithm, then that algorithm exhibits perfect recall" }.
...
In Figure~\ref{fig:results:fashion-mnist-scaling}, which shows results for Fashion-mnist, we observe that as cardinality increases, the CAKES algorithms (GreedySieve in blue, Repeated $\rho$-NN in green, and Sieve in red) exhibit higher throughput than na\"{i}ve linear search (plotted in orange).
...
For this dataset, Sieve, plotted in red, is the highest-throughput CAKES algorithm for all cardinalities.

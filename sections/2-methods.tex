...
For Repeated $\rho$-NN search, we begin by performing \textit{tree-search} with a search radius $r$ equal to the radius of the root cluster divided by the cardinality of the dataset.
...
We now perform \textit{leaf-search} to find the $k$ nearest neighbors among the points in the clusters found during the previous steps.

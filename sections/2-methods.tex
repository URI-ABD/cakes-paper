...
Let $\ell$ and $r$ denote the cluster's left and right poles (see Section~\ref{subsubsec:methods:cluster-partitioning}), $q$ denote the query, and $\rho$ denote the search radius, as shown in Figure [ADD FIGURE].
...
Now let $m$ be the midpoint of $\overline{\ell r}$. Let $d$ denote the distance from $q$ to the perpendicular bisector of $\overline{lr}$, as shown in Figure [ADD FIGURE]. Since we assign a point in the parent cluster to the left child if it is closer to the left pole, if $d > \rho$, 
...
We note that $d = ||mq'|| = ||mr||-||q'r||$. Since $m$ is the midpoint of $\overline{\ell r}$, we have that $||mr|| = \tfrac{||\ell r||}{2}$, so it remains to find $||q'r||$. To do this, we use the Law of Cosines with $\triangle \ell rq$. Let $\theta$ denote $\angle lrq$, as shown in Figure [ADD FIGURE]. By the Law of Cosines, we have that 
\begin{equation} \text{cos}(\theta) = \tfrac{||\overline{qr}||^2 + ||\overline{\ell r}||^2 - ||\overline{q\ell}||^2}{2||\overline{\ell r}||}.\end{equation}. 
On the other hand, since $\triangle q'qr$ is a right triangle, 

\begin{equation} \text{cos}(\theta) = \tfrac{||\overline{q'r}||}{||\overline{qr}||}.\end{equation}


To check this, it suffices to verify that \begin{equation}(||\overline{q\ell}|| + ||\overline{qr}||)(||\overline{q\ell}||- ||\overline{qr}||) \leq 2||\overline{\ell r}||\rho. \end{equation} 

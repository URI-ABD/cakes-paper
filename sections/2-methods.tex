...
In ~\cite{ishaq2019clustered}, we claimed that the complexity of coarse search for $\rho$-NN is $\mathcal{O}(\log{2}m)$, 
...
$\mathcal{O}\left(\log_2{m} + k\left(\frac{r_i\left(\frac{k}{|C_i|}\right)^{\frac{1}{d_i}}+ 2r_c}{r_i\left(\frac{k}{|C_i|}\right)^{\frac{1}{d_i}}}\right)^d\right)$ 
time complexity, where $m$ is the metric entropy of the dataset, $d$ is the dimensionality of the dataset, $r_i$ is the radius of some cluster $C_i$

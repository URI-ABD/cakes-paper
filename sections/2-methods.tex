...
(i.e. arises from the same generating phenomenon) as the rest of the data. We also rely on the same technical assumption about the self-similarity 
of the dataset made in \cite{yu2015entropy}: if call the number of points within a given 
...
In ~\cite{ishaq2019clustered}, we claimed that the complexity of coarse search is $\mathcal{O}(\log{2}m)$, 
where $m$ is the metric entropy of the dataset. 
...
Because we assume that the query point is sampled from 
the same distribution as the rest of the data, we can assume that the local fractal dimension around the query
is not significantly different from the mean local fractal dimension of the dataset. If the local fractal dimension 
were perfectly uniform throughout the entire dataset, then 
the use of the factor from Equation \ref{2} would guarantee that it only takes 
one iteration of $\rho$-NN to find $k$ points. In practice, [is this the 
way I want to be going with this??]
...
~\cite{yu2015entropy} showed that $|F| \leq \gamma |B(q, r')|\left(\frac{r'+ 2r_c}{r'}\right)^d$. By definition of 
...
average local fractal dimension over all regions of the dataset does not differ significantly from the local fractal dimension around any particular point.
...
and the cardinality of the ball centered at the root cluster). In this algorithm, we perform $\rho$-nearest neighbors search at equal to the radius of the cluster tree divided by
...
Since $d_0$, $|C_0|$, $r_0$, and $k$ are all known values, we can solve equation ~\ref{4} for $r'$ to get
...
Our modified version of this algorithm (see Algorithm 4) finds the smallest index $i$ such 
...
\subsubsection{Complexity of Sieve, Sieve with Separate Centers, and Greedy Sieve}

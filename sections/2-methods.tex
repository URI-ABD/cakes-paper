...
With this approach, we begin by taking a random sample of up to 10\% of the data. This sample 
...
We then split the rest of the dataset into shards, based on the user-defined memory limit, and use the algorithm identified in the previous step to perform $k$-NN search on the first shard. 
...
For datasets which \emph{do} fit in memory, we use the same approach to auto-tuning, but, since the entire remainder of the dataset fits in memory, we use everything 
but that initial 10\% sample as the remaining shard for $\rho$-nearest neighbors search.

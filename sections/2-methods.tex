...
\end{equation}
where $p$ is a dataset point and $B(p, r)$ is the set of points within radius $r$ of $p$.
For a cluster $\mathcal{C}$ with center $c$ and radius $r$, we define its LFD as $\text{LFD}(c, r, r/2)$.
If the median LFD of clusters at most (and particularly high) depths in the tree is $\leq 3$, we say the dataset has ``low LFD''.
...
For a cluster $\mathcal{C}$ with center $c$ and radius $r$, we define $\delta_\mathcal{C} = f(q, c)$, $\delta^{+}_\mathcal{C} = \delta_\mathcal{C} + r$, and $\delta^{-}_\mathcal{C} = \min(0, \delta_\mathcal{C} - r)$.


\subsubsection{Repeated \texorpdfstring{$\rho$}{p}-NN}
\label{sec:methods:knn-search:repeated-rnn}
This algorithm builds on $\rho$-NN search from CHESS~\cite{ishaq2019clustered}, which finds all points within a radius $\rho$ around a query.
Repeated $\rho$-NN begins by using \textsc{TreeSearch} (Alg~\ref{alg:methods:rnn-search:tree-search}) on the root cluster $\mathcal{R}$ with the search radius $r$ equal to the radius of $\mathcal{R}$ divided by $|\textbf{X}|$.
...
\end{gather}
where $\hat{r}$ is the mean cluster radius, $\mathcal{N}_{\hat{r}}(\textbf{X})$ is the minimum number of clusters of radius $\hat{r}$ needed to cover $\textbf{X}$.

...
The complexity of Sieve Search is dominated by the complexity of our modified Quicksort Partition algorithm,
which, like the original Quicksort partition algorithm described in~\cite{10.1093/comjnl/5.1.10}, has $\mathcal{O}(n)$ complexity, 
where $n$ is the length of the input list. While the cardinality of clusters is greater than $k$, 2 is bounded 
above by 2, as $Q$ will contain at most two clusters (assuming the worst case, which is a balanced clustering). 
Once the cardinality of clusters in $Q$ is less than or equal to $k$, the cardinality of $Q$ is bounded above 
by $2k$, since, in the worst case, every cluster is a non-leaf and  

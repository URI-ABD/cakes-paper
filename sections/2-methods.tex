...
% Najib: Add our own plots of local fractal dimension instead of referencing figures from the CHESS paper. Then, rewrite this paragraph.
...
Najib We define a \emph{cluster} as a set of points with a \emph{center} and a \emph{radius}.
The \emph{center} is the geometric median of a sample of points in the \emph{cluster}, and so it is a real data point.
The \emph{radius} is the maximum distance of any point in the cluster to the \emph{center}.
Each non-leaf cluster has two child clusters in much the same way that a node in a binary tree has two child nodes.
We estimate local fractal dimension at the scale of the cluster radius and half the cluster radius. 
This allows us to rewrite Equation \ref{eq:methods:lfd_original} as simply: 
\begin{equation} 
\log_2 \frac{|B_X(q, r)}{|B_X(q, r/2)|}
\end{equation}


% Najib: Rewrite this paragraph. Old paper defined it as ... but we define it for the hierarchical clustering scheme as ... DONE. (let me know if this okay)

In~\cite{yu2015entropy}, metric entropy for a given cluster radius $r_c$ was defined as the number of clusters of radius $r_c$ needed to cover all data. In this paper, we define the metric entropy $\mathcal{N}_{\hat{r_c}}(X)$ of a data set $X$ for the hierarchical clustering scheme as the number of leaf clusters in the tree.
...
We start by building a divisive hierarchical clustering of the dataset using CLAM.
...
We build a cluster tree with this sample using Algorithm~\ref{alg:methods:clustering:partition}, and select all clusters at some user-specified depth in the tree, with a default depth of 10.
...
% Najib: For this figure, I want the red, green and blue lines/braces to be thinner. Perhaps 1/2 to 2/3 as thick.
\begin{figure}[ht!]
    \centering
    \includegraphics[scale=0.5]{images/geometry/deltas.png}
    \caption{$\delta$, $\delta_{\max}$, and $\delta_{\min}$ defined for cluster $C$ and query $q$.}
...
when a cluster overlaps with the query ball, instead of always proceeding with both of its children, we proceed only with those children which could contain points in the query ball. The full algorithm is described in Algorithm~\ref{alg:methods:rho-nn-search}.


% Najib: Merge Figures 2 and 3. There's too much redundancy between them.
% \begin{figure}[ht!]
%     \centering
...
% Najib: Visually, ``m'' is not centered on the line segment. It's a little bit to the right.
\begin{figure}[ht!]
    \centering
...
Let $\ell$ and $r$ denote the cluster's left and right poles (see Section~\ref{subsubsec:methods:cluster-partitioning}), $q$ denote the query, and $\rho$ denote the search radius, as shown in Figure~\ref{fig:methods:overlapping-children-3}.
...
In the caculations that follow, we overload notation a bit and use $\overline{xy}$ to mean the length of the line segment between $x$ and $y$. We note that $d = \overline{mq'} = \overline{mr \vphantom {q'}}-\overline{q'r}$. Since $m$ is the midpoint of $\overline{\ell r}$, we have that $\overline{mr} = \tfrac{\overline{\ell r}}{2}$, so \begin{equation*} d = \tfrac{\overline{\ell r}}{2} - \overline{q'r}.
\label{eq:methods:d-expression}
\end{equation*}



It remains to find $\overline{q'r}$. To do this, we use the Law of Cosines with $\triangle \ell rq$. Let $\theta$ denote $\angle lrq$, as shown in Figure~\ref{fig:methods:overlapping-children-3}. By the Law of Cosines, we have that 
...
% Najib: move is.Leaf check to the beginning of the algorithm because it's the base base case. DONE (confirm this is correct)
...
\begin{algorithm} 
    ...
    \end{algorithmic}
    \label{alg:methods:rho-nn-search}
\end{algorithm}
...
where $\hat{r_c}$ is the \textit{mean} cluster radius of leaf clusters, $\mathcal{N}_{\hat{r_c}}(X)$ is the metric entropy at that radius, $B_X(q,r)$ is a ball of data points around the query $q$ at search radius $r$, and $d$ is the local fractal dimension around the query in a region of radius $r$.
...
In this algorithm, we perform $\rho$-nearest neighbors search starting with a small search radius,
...
Because we have that for any cluster $C$, $C.\delta \leq C.\delta_{max}$, representing the cluster center separately from the rest of the points causes the threshold $\tau$ to decrease more quickly, thus allowing us to eliminate some clusters from $Q$ earlier in the process.  and repeatedly increasing the radius until we find $k$ neighbors.
...
In this algorithm, we begin by letting $Q$ be a list of clusters and points. 
{ \color{red} We represent each cluster twice in $Q$ -- once as just its center and once as the rest of its points. $Q$ is initialized with two elements: the root cluster and the root cluster's center. }

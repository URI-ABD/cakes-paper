...
We require that the distance function be symmetric (i.e., $f(x, y) = f(y, x) \ \forall \ x, y \in \textbf{X}$) and has identity of indiscernibles (i.e. $f(x, y) = 0 \iff x = y$).
...
For example, Euclidean, Levenshtein~\cite{levenshtein1966binary} and Dynamic Time Warping (DTW)~\cite{muller2007dynamic} distances are all distance metrics while Cosine distance is not a metric because it does not obey the triangle inequality.
...
Starting from a root cluster containing the entire dataset, we repeat this procedure until each leaf contains only one datum, or we meet some other user-specified stopping criteria, for example, a minimum cluster radius, a minimum cluster cardinality, a maximum tree depth, etc.
...
\begin{algorithm} % enter the algorithm environment
    \caption{Partition(\emph{C, criteria})} % give the algorithm a caption

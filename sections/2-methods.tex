...
In this manuscript, we are primarily concerned with $k$-nearest neighbors in a finite-dimensional space. Given a dataset $\textbf{X} = \{x_1 \dots x_n\}$, we define a \emph{point} or \emph{datum} $x_i \in \textbf{X}$ as a singular observation (e.g., the genome of 
...
In addition to these constraints, if the distance function obeys
the triangle inequality, i.e., it is a \emph{distance metric}, then we guarantee that CAKES has perfect recall. 
...
manifold within the 8,575-dimensional embedding space. The \emph{local fractal dimension} is an approximation of the dimensionality of that lower-dimensional manifold in the ``vicinity'' of a given point.
...
The point $l$ which is responsible for that radius (i.e., the furthest point from the center) is designated the \emph{left pole}. The point $r$ which is furthest
...
Assuming a balanced tree and thus $\mathcal{O}\log_2(n)$ depth, this approach had $\mathcal{O}(n\log_2(n)$ memory cost. We note that while a balanced tree is classically a best-case scenario (e.g. binary search), it represents a worst-case scenario for space complexity in this application; the space complexity of a completely unbalanced (linear) tree would be $\mathcal{O}(n)$.

One may improve this memory cost to $\mathcal{O}(n)$ by storing only these indices at 
...
store the cluster's cardinality and an \emph{offset} to access the cluster's points from the dataset. A left child has 
the same offset as its parent cluster, and the corresponding right child has an offset equal to the left child's offset
plus the left child's cardinality. With no additional memory cost nor time cost for retrieving points during search, 
...
While it may be tempting to assume that cluster radii decrease with each application of Partition (refer to Algorithm \ref{alg:partition}), unfortunately, this assumption is incorrect. 
...
$d$ partitions, where $d$ is the fractal dimensionality of the dataset. 
...
In this case, Partition will select a maximally distant pair of points to use as poles; without loss of generality, 
say it chooses two points at the extrema one of the $d$ axes. 
After one partition, the maximum pairwise distance along that axis will be
bounded above by $R$. 
...
Thus, after at most $d$ partitions, the
...
See Figure~\ref{fig:methods:scaling_behavior} for an example. 
...
Note that, in practice, we never see a balanced clustering. Partition  produces unbalanced trees due to the varying density of the sampling 
in different regions of the manifold and the low dimensional ``shape'' of the manifold. Further, in practice, the cluster radii decrease by a factor much larger than 
$\frac{\sqrt{2}}{2}$ with every partition; the upper bound of $d$ partitions is seldom realized. 


\begin{figure}[ht!]
    \centering
    \includegraphics[width=3.4in]{images/geometry/geometry.pdf}
    \caption{Scaling behavior of radii on a uniform-density disk, which represents the worst-case scenario for a two-dimensional distribution. 
...
This approach also leverages the fact that $\rho$-nearest neighbors search at the radius of the $k$-th farthest neighbor 
is typically much faster than $k$-nearest neighbors search.
...
With this approach, we begin by taking a random sample of no more than 10\% of the data. This sample 
is considered the first shard. 
...
then consider all clusters at some user-specified depth in the tree, with a default depth of 10. Using these clusters' centers as queries, and a user-specified value of $k$, we record the time taken for $k$-NN search using each of the four algorithms described in Section~\ref{subsec:methods:knn-search}. We select the algorithm which had the lowest total search time over all the queries. 
We then split the rest of the dataset into shards based on the user-defined memory limit. 


Sharded search then proceeds as follows. We use previously determined $k$ algorithm on the first shard. 
We use the distance from the $k$-th farthest neighbor to perform $\rho$-NN search on each subsequent shard. 
After searching each shard, we keep only the $k$-nearest neighbors so far, potentially decreasing the distance 
from the $k$-th farthest neighbor for search on the next shard. 
...
Finding the precise optimal ratio between the size of the first shard and the size of the remaining shards, as well as computing the expected level of error between the distance from the $k$-th closest neighbor in the first shard and the actual $k$-th closest neighbor, are topics for future work. For datasets which \emph{do} fit in memory, we use the same approach to auto-tuning, but, since the entire dataset fits in memory, there is only one shard. 
...
The complexity of Sieve and Sieve with Separate Centers is dominated by the expected 
cost of Quicksort-inspired partition algorithm we use to calculate thresholds. This method is linear in the size of the input list $Q$, so we must estimate the size of $Q$ on each iteration. 

We rely on the assumption that when the cardinalities of clusters is within a low factor of $k$, say $2k$, the distribution of clusters will mimic the distribution of points in the dataset. 
...
In particular, we assume that the number of clusters which overlap the query ball is bounded above by 2$d$, where $d$ is the local fractal dimension of the dataset near the query point, as we could have a cluster overlapping the query ball at each end of each of the $d$ axes.

In the worst case scenario, where the data are distributed with uniform density in their embedding space, it takes at most $\log_2{\frac{n}{2k}}$ iterations of Sieve to partition clusters enough that their cardinalities are at most $2k$. However, since 
high cardinality clusters occur only at low depths in the tree, the length of $Q$ for 
those iterations is relatively short. 


After the first $\log_2{\frac{n}{2k}}$ iterations, we have that the cardinalities of clusters in $Q$ are at most $2k$, at which point, based on our assumption, we have that the length of $Q$ is close to $2d$.
Therefore, the asymptotic complexity of the sieve algorithms is $\mathcal{O}(log_2(\frac{n}{d})$.

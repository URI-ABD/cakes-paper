...
While ordinarily we would compute LFD by comparing cardinalities of two balls with two different radii centered at \textit{the same} point, in order to estimate $\rho_k$, we instead compare the cardinality of a ball \textit{centered at the query} of radius $\rho_k$ to the mean cardinality, $\hat{|\mathcal{C}|}$, of clusters in $\mathcal{Q}$ at a radius equal to the mean of their radii, $\hat{r}$.
...
$\mathcal{Q}$ initially contains only $(\mathcal{R}, \delta^{+}_{\mathcal{R}}, |\mathcal{R}| - 1)$ while $\mathcal{H}$ initially contains only the center of $\mathcal{R}$.

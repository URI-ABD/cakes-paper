...
We use the term \textit{approximate} to refer to algorithms that have imperfect recall, while \textit{exact} refers to algorithms that have perfect recall as measured against a brute-force search.
...
In this paper, we introduce CAKES, a set of three entropy-scaling algorithms for $k$-NN search, implemented in Rust, extending CHESS~\cite{ishaq2019clustered} to $k$-NN search.
Sections~\ref{sec:methods:knn-search:repeated-rnn-complexity} and~\ref{sec:methods:knn-search:complexity-of-sieve-methods} provide a theoretical analysis.

...
Given a distance function on a dataset, $k$-nearest neighbors ($k$-NN) search returns the $k$ closest points to a query. We call algorithms with imperfect recall \textit{approximate} and those matching brute-force recall \textit{exact}. Most fast methods are approximate~\cite{gao2023high}; while sufficient for some applications, there remains demand for \textit{fast exact} search~\cite{ukey2023survey}.

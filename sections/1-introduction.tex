...
Naive implementations of kNN, whose time complexity is linear in the dataset's cardinality, prove prohibitively slow for large datasets.
...
There are many algorithms that perform well here, such as FM-Index~\cite{ferragina2000opportunistic}, which are less applicable to approximate search.
...
In any case, $k$-NN is commonly used for inference;
the \emph{prediction} of a class for a query is obviously not guaranteed to be correct.
When we refer to \emph{exact} $k$-NN, we refer to precisely the $k$ nearest neighbors being returned;
...
There is some evidence that non-metrics such as cosine distance perform poorly for $k$-NN in biomedical settings~\cite{hu2016distance};

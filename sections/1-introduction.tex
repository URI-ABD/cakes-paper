...
% Najib: We should mention datasets in this order: neural -network embeddings, biological, radio-ml, and then astronomical.
...
% Najib: Switch the next paragraph for this:
...
% Najib: The previous sentence needs a citation.
...
% Najib: As the cardinalities and dimensionalities ...
...
% Najib: Rework this paragraph. I don' want any mathematically notation in the introduction, particularly because this is repeated in Methods, The Search Problem.
...
% Najib: we reserve the term \emph{approximate} for search algorithms which do not exhibit perfect recall.
...
% Najib: Same here with the math notation.
...
% Najib: When we refer to an \emph{exact} $k$-NN algorithm, we mean an algorithm which exhibits perfect recall when compared to a na\"{i}ve linear search.
...
% Najib: Remove/replace the word ``pervasive'' here. It has a negative connotation.
...
Na\"{i}ve implementations of $k$-NN, whose time complexity is linear in the dataset's cardinality, prove prohibitively slow for large datasets. % Najib: because the dataset's cardinality is growing exponentially.
% Najib: While algorithms for fast $k$-NN on large datasets DO exist, most of them are approximate and do not exploit the geometric and topological structure inherent in SUCH datasets.
...
% Najib: Further, such algorithms are often approximate and while approximate search MAY be sufficient for some applications, the need for efficient AND \emph{exact} search remains.
...
This is especially true when classes are not well-separated. % TODO: Cite something which says this.
...
% Najib: Cite HNSW here. Also, add ANNOY.

...
On the Random dataset, CAKES's algorithms' throughputs decrease linearly as the multiplier increases, while with Sift, Depth-First Sieve and Breadth-First Sieve both exhibit nearly constant throughput.
...
We also note that the CAKES algorithm with the best throughput varies by dataset.
...
When we view these results in light of the LFD plots in Section~\ref{sec:results:lfd-of-datasets}, we realize that Repeated $\rho$-NN appears to be the fastest CAKES algorithm on datasets where a large proportion of the dataset has a very low LFD (i.e., near $2$), such as Glove-25 and Silva. 

...
Thus, Random shows that CAKES is designed to exploit manifold structure when it exists, and should not be used on datasets that do not obey the manifold hypothesis.
...
We also include benchmarks on the SilVA-18S rRNA and Radio-ML datasets.
...
We use the 10dB SNR subset with 97,304 samples and use Dynamic Time Warping~\cite{muller2007dynamic} as the distance metric.
For both datasets, we hold out 1,000 random samples as queries and build the tree on the remaining samples.
These were conducted on an Intel Xeon E5-2690 v4 CPU @ 2.60GHz with 512GB RAM running Manjaro Linux 5.15.164-1-MANJARO with Rust compiler version 1.83.0.
...
In order to study scaling behavior as cardinality increases, we synthetically augment the ANN-Benchmarks datasets by generating new points by applying small random perturbations to existing points in the original dataset.
...
\begin{figure}[t]
    ...
    \caption{Local fractal dimension vs. cluster depth across six datasets; note the different y-axis. The x-axis is tree depth; the y-axis is LFD at that depth. We plot the 5th, 25th, 50th, 75th, and 95th percentiles, and min/max. The Fashion-MNIST, Glove-25, Sift and Silva datasets are real-world datasets and exhibit low LFD indicating the presence of manifold structure. The Random dataset has high LFD, indicating lack of manifold structure. The RadioML dataset is also a randomly generated dataset, but with a much more elaborate generation process that imparts some manifold structure at three distinct radial scales, indicated by the three peaks in LFD at different depths in the tree.}
...
Tables~\ref{tab:results:qps-and-recall-fmn}, \ref{tab:results:qps-and-recall-glove}, \ref{tab:results:qps-and-recall-sift}, and \ref{tab:results:qps-and-recall-random} show the throughput (QPS) and recall for each algorithm on each of the vector datasets.
...
All benchmarks were run with $k=10$, the throughput is averaged over all queries, and recall is computed using a na\"ive linear scan.

HNSW, ANNOY, and FAISS have various hyper-parameters that can be tuned to trade off throughput, recall, and indexing time.
We allow for a hyper-parameter search (in the ranges recommended by the documentation accompanying the respective libraries) to find the highest throughput configuration that achieves perfect recall (or as close to it as possible) for each dataset and cardinality.
To make the comparison fair, we also allow CAKES to select the optimal algorithm among the three CAKES algorithms for each dataset and cardinality by using a small number of the centers of clusters at low depth as representative queries.

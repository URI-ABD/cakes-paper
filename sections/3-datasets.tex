...
We use the 10dB SNR subset with 97,304 samples and Dynamic Time Warping~\cite{muller2007dynamic} as the distance metric.
For both datasets, we hold out 1,000 random samples as queries and build the tree on the remaining samples.
These were conducted on an Intel Xeon E5-2690 v4 CPU @ 2.60GHz with 512GB RAM running Manjaro Linux 5.15.164-1-MANJARO.
...
All benchmarks were run with $k=10$, throughput is averaged over all queries, and recall is computed using a na\"ive linear scan.

HNSW, ANNOY, and FAISS have various hyper-parameters that can be tuned to trade off throughput, recall, and indexing time.
We allow for a hyper-parameter search (in the ranges recommended by the documentation accompanying the respective libraries) to find the configuration with the highest throughput that still achieves perfect recall (or as close to it as possible) for each dataset and cardinality.
To make the comparison fair, we also allow CAKES to select the fastest of its algorithms for each dataset and cardinality by testing each with a small number of the centers of clusters at low depth as representative queries.

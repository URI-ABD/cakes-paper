...
The time complexity of CAKES's algorithms scales with the metric entropy and fractal dimension of datasets, and they perform best when data occupy a low dimensional manifold -- even when those data occupy a very high dimensional embedding space. 
We demonstrate that on low-dimensional manifolds, CAKES exhibits sub-linear performance on large datasets, and that CAKES's performance is sometimes constant in the size of the dataset. 
...
Even when data do not occupy a metric space, CAKES exhibits significantly higher recall than state-of-the-art $k$-NN search algorithms. 
Additionally, we show that indexing and tuning time for CAKES is orders of magnitude faster than those approaches.
